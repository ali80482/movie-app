...
const MovieApp = () => {
  ...
  const movieLatestData = useSelector(
    (state) => state?.movie?.recommendedMovie?.components
  );
  ...
  return (
    <>
      <div className="visibleStyle">
        <SetOrder changeOrder={changeOrder} orderSelect={orderSelect} />
        <div className="clearStyle" />
        <Card movie={movie} handleShowPopupData={handleShowPopupData} />
        <div className="clearStyle" />
      </div>
      <Popup
        showPopup={showPopup}
